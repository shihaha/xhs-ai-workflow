...
Task 4: implementation complete; bounded independent review pending. TDD RED was the expected missing media API/worker import failure. The app now owns one serialized durable media worker with startup recovery and shutdown admission fencing; strict submission accepts only trusted IDs, generic Jobs cannot mutate reserved media work, and list/detail plus separate media health facts are wired through the shared service. Final focused verification: 95 passed. Full backend regression: 1361 passed, 2 guarded live skips; no failures. Live Bailian remains not_run.
