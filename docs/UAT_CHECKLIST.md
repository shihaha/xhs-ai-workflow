...
- Second independent account: **partially trusted, shop evidence blocked** — one distinct account has a trusted profile and 10 persisted latest-note sample facts. Android later observed 18 deduplicated product links, but those links remained only in the discovery process: the job has 66 screenshots, 66 UI hierarchies, zero `shop_collection_result` artifacts and zero reusable persisted `source_url` fields. Current trusted product sample is `0/3`, not `3/3`; no phone rescan was performed to conceal this gap.
