...
- Bailian image/vision live gate: **not_run** — explicit `BAILIAN_MEDIA_LIVE_TEST=1` plus a trusted local key and explicit image/vision model configuration were not supplied. The default guarded test creates no successful run or generated file.
...
- This controlled path is software evidence. Real Bailian media remains `not_run` until the explicit guarded gate is run with locally configured authority.
