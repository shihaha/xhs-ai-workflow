...
- Bailian image/vision live gate: **real image path passed; visual format failed; rerun pending** — the third opted-in run completed wan2.6 submission, polling, real download, byte validation and managed material persistence. qwen-vl-max returned valid JSON with self-selected Chinese keys instead of the exact `VisualAssessment` contract, so strict validation failed. The adapter now sends a fixed system instruction containing the exact JSON Schema and still rejects renamed/extra keys; a vision-only or complete live rerun must prove visual success.
...
- This controlled path is software evidence. Real Bailian image generation is now proven through managed material; real visual analysis remains unproven until the guarded gate passes on a fresh rerun.
