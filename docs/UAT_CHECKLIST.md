...
- Bailian image/vision live gate: **provider image generated, local completion failed, rerun pending** — after the first run exposed the obsolete route/body, the second opted-in run submitted and polled wan2.6 to `SUCCEEDED` and received a real image URL. Local processing then rejected the official usage object's `size: "1280*1280"` metadata before managed image persistence, so vision was not reached. The image client now keeps only the official numeric usage counters under the unchanged strict validator; a fresh full rerun is still required.
...
- This controlled path is software evidence. Bailian has now generated a real remote image, but managed local image completion and real visual analysis remain unproven until the guarded gate passes on a fresh rerun.
