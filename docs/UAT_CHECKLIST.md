...
- Bailian image/vision live gate: **attempted, failed, rerun pending** — the first opted-in wan2.6-t2i generation reached Bailian but submission returned HTTP 400 `InvalidParameter` / `url error`; the request ID is retained in local live output. This exposed an obsolete client route/body and did not create a successful generated image or reach vision analysis. A minimal contract fix is implemented; only a fresh real rerun may change this item to passed.
...
- This controlled path is software evidence. The first real Bailian attempt failed at image submission as recorded above; real generation and vision success remain unproven until the guarded gate passes on a fresh rerun.
