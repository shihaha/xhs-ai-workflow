...
import {
  ...
  fetchProducts, regenerateContentItem, reviewContentItem, type ContentItem, type ContentPackage,
  type Product,
} from "../api/client";

type StudioData = { products: Product[]; items: ContentItem[]; packages: ContentPackage[] };
...
const defaultLoad = async () => { const [products, items, packages] = await Promise.all([fetchProducts(), fetchContentItems(), fetchContentPackages()]); return { products, items, packages }; };

export function ContentStudioPage({ loadStudio = defaultLoad, addMaterial = addProductMaterial, createItem = createContentItem, reviewItem = reviewContentItem, regenerateItem = regenerateContentItem, exportItem = exportContentItem }: ContentStudioPageProps) {
  ...
  return <main className="workbench-page" id="main-content"><header className="page-heading"><p className="eyebrow">Human-reviewed production</p><h1>Content studio</h1><p>Create, review, regenerate and export a local pending-publication package. This workbench never publishes automatically.</p></header>
    ...
    {products.length ? <section className="operator-panel"><div className="panel-heading"><h2>Products and materials</h2><p>{products.length} returned</p></div><div className="record-stack">{products.map(product => <ProductWorkbench key={product.id} product={product} addMaterial={addMaterial} createItem={createItem} run={run} />)}</div></section> : null}
    {items.length ? <section className="operator-panel"><div className="panel-heading"><h2>Review queue</h2><p>{items.length} items</p></div><div className="record-stack">{items.map(item => <ContentReview key={item.id} item={item} reviewItem={reviewItem} regenerateItem={regenerateItem} exportItem={exportItem} run={run} />)}</div></section> : null}
...
function ProductWorkbench({ product, addMaterial, createItem, run }: { product: Product; addMaterial: Action; createItem: (payload: Record<string, unknown>) => Promise<unknown>; run: (action: () => Promise<unknown>, success: (value: unknown) => string) => Promise<void> }) {
...
function ContentReview({ item, reviewItem, regenerateItem, exportItem, run }: { item: ContentItem; reviewItem: Action; regenerateItem: Action; exportItem: Action; run: (action: () => Promise<unknown>, success: (value: unknown) => string) => Promise<void> }) {
  ...
  return <article className="studio-record"><header><div><h3>{revision.title}</h3><code>{item.id} · revision {revision.number}</code></div><span className={`state state--${item.status}`}>{item.status}</span></header><p className="draft-body">{revision.body}</p><p className="evidence-line">Sources: {revision.source_evidence_ids.join(", ")}</p>{item.status === "review" ? <><label>Review note<input aria-label={`Review note for ${revision.title}`} value={note} onChange={event => setNote(event.target.value)} /></label>{item.image_material_ids.map(id => <label key={id}>Visual check {id}<input aria-label={`Visual check for ${id}`} value={checks[id] ?? ""} onChange={event => setChecks(current => ({ ...current, [id]: event.target.value }))} /></label>)}</> : null}<div className="button-row">{item.status === "review" ? <><button type="button" onClick={() => void run(() => reviewItem(item.id, { decision: "reject", expected_revision_id: revision.id, actor: "local-operator", note, visual_checks: [] }), () => "Rejection persisted. Persisted facts were reloaded.")}>Reject {revision.title}</button><button type="button" disabled={!note || visuals.some(check => !check.observation)} onClick={() => void run(() => reviewItem(item.id, { decision: "approve", expected_revision_id: revision.id, actor: "local-operator", note, visual_checks: visuals }), () => "Approval persisted. Persisted facts were reloaded.")}>Approve {revision.title}</button></> : null}{item.status === "rejected" ? <button type="button" onClick={() => void run(() => regenerateItem(item.id, { expected_revision_id: revision.id }), () => "Regeneration request completed. Persisted facts were reloaded.")}>Regenerate {revision.title}</button> : null}{item.status === "approved" ? <button type="button" onClick={() => void run(() => exportItem(item.id, { expected_revision_id: revision.id }), value => `Package available: ${(value as { path?: string }).path ?? "path not returned"}`)}>Export {revision.title}</button> : null}</div><p className="boundary-note">Export creates a local pending-publication ZIP only. No account action is performed.</p></article>;
}
