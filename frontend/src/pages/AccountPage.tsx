...
type AnalysisPayload =
  ...
  | { analysis_type: "account_opportunity"; account_user_ids: string[]; evidence_ids: string[] };
...
export function AccountPage({
  ...
  useEffect(() => {
    if (!activeCollectionId || !activeCollection || terminal.has(activeCollection.status) || activeCollection.polls >= maxPolls) return;
    let cancelled = false;
  ...
  return <main className="workbench-page" id="main-content">
    ...
    <section className="operator-panel">
      ...
      {accountNoteEvidence.length ? <div><h3>Trusted selectable note evidence</h3><ul>{accountNoteEvidence.map(item => <li key={item.evidence_id}><code>{item.evidence_id}</code> · trusted account-note input</li>)}</ul><p className="field-help">Trusted account-note evidence can ground analysis claims. It does not replace exact shop N/N verification required for opportunity creation.</p></div> : null}
    </section>
    ...
    <div className="workflow-grid">
      ...
      <section className="operator-panel"><div className="panel-heading"><h2>Evidence-bound analysis</h2></div>{evidence.length === 0 ? <p className="panel-empty">No eligible evidence returned.</p> : <form className="action-form" onSubmit={event => { event.preventDefault(); void act(async () => { await createAnalysis({ analysis_type: "account_report", account_user_id: account.user_id, account_user_ids: [], evidence_ids: selected }); try { await reloadFacts(); } catch { setActionError("Action completed, but persisted facts could not be reloaded."); } return "Account analysis request completed. Refresh to inspect the persisted result."; }); }}><fieldset><legend>Persisted evidence</legend>{evidence.map(item => <label className="check-label" key={item.evidence_id}><input aria-label={item.evidence_id} checked={selected.includes(item.evidence_id)} type="checkbox" onChange={event => setSelected(current => event.target.checked ? [...current, item.evidence_id] : current.filter(id => id !== item.evidence_id))} />{item.evidence_id} · {item.kind}{item.kind === "account_note" && item.eligible_for_opportunity ? " · trusted selectable input" : item.eligible_for_opportunity ? " · deep-verified" : ""}</label>)}</fieldset><div className="button-row"><button disabled={pending || selected.length === 0} type="submit">Generate account report</button><button disabled={pending || selected.length === 0} type="button" onClick={() => void act(async () => { await createAnalysis({ analysis_type: "account_opportunity", account_user_ids: [account.user_id], evidence_ids: selected }); try { await reloadFacts(); } catch { setActionError("Action completed, but persisted facts could not be reloaded."); } return "Opportunity analysis request completed. Open Opportunities to inspect persisted cards."; })}>Generate opportunity analysis</button></div></form>}</section>
    </div>
