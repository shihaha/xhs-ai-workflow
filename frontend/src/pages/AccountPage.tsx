...
export function AccountPage({
  ...
  return <main className="workbench-page" id="main-content">
    ...
    <div className="workflow-grid">
      ...
      <section className="operator-panel"><div className="panel-heading"><h2>Evidence-bound analysis</h2></div>{evidence.length === 0 ? <p className="panel-empty">No eligible evidence returned.</p> : <form className="action-form" onSubmit={event => { event.preventDefault(); void act(async () => { await createAnalysis({ analysis_type: "account_report", account_user_id: account.user_id, account_user_ids: [], evidence_ids: selectedEvidenceIds }); try { await reloadFacts(); } catch { setActionError("Action completed, but persisted facts could not be reloaded."); } return "Account analysis request completed. Refresh to inspect the persisted result."; }); }}><fieldset><legend>Persisted evidence</legend>{evidence.map(item => { const selectable = item.kind !== "account_note" || item.eligible_for_opportunity; return <label className="check-label" key={item.evidence_id}><input aria-label={item.evidence_id} checked={selectable && selected.includes(item.evidence_id)} disabled={!selectable} type="checkbox" onChange={event => setSelected(current => event.target.checked ? [...current, item.evidence_id] : current.filter(id => id !== item.evidence_id))} />{item.evidence_id} · {item.kind}{item.kind === "account_note" && item.eligible_for_opportunity ? " · trusted selectable input" : item.kind === "account_note" ? " · stale or untrusted · human verification required" : item.eligible_for_opportunity ? " · deep-verified" : ""}</label>; })}</fieldset><div className="button-row"><button disabled={pending || selectedEvidenceIds.length === 0} type="submit">Generate account report</button><button disabled={pending || selectedEvidenceIds.length === 0} type="button" onClick={() => void act(async () => { await createAnalysis({ analysis_type: "account_opportunity", account_user_ids: [account.user_id], evidence_ids: selectedEvidenceIds }); try { await reloadFacts(); } catch { setActionError("Action completed, but persisted facts could not be reloaded."); } return "Opportunity analysis request completed. Open Opportunities to inspect persisted cards."; })}>Generate opportunity analysis</button></div></form>}</section>
    </div>
