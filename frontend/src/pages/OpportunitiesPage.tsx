...
export function OpportunitiesPage({ loadOpportunities = defaultLoad, createAnalysis = postAnalysis, reviewOpportunity = postReview }: OpportunitiesPageProps) {
  ...
  return <main className="workbench-page" id="main-content">
    ...
    {opportunities.length === 0 ? <section className="message-panel" role="status"><h2>暂时没有跨账号候选</h2><p>只有至少两个证据完整的账号通过服务端验证，分析结果才会保存为候选。</p></section> : <ol className="opportunity-list">{opportunities.map(opportunity => <li className="opportunity-record" key={opportunity.id}>
      ...
      {opportunity.review_status === "pending_review" ? <div className="action-form"><label>拒绝原因<textarea aria-label={`拒绝 ${opportunity.title} 的原因`} value={rejectionReasons[opportunity.id] ?? ""} onChange={event => setRejectionReasons(current => ({ ...current, [opportunity.id]: event.target.value }))} /></label><div className="button-row"><button disabled={pending} type="button" onClick={() => void act(async () => { await reviewOpportunity(opportunity.id, { decision: "approve" }); return `已批准：${opportunity.title}`; })}>批准：{opportunity.title}</button><button disabled={pending || !(rejectionReasons[opportunity.id] ?? "").trim()} type="button" onClick={() => void act(async () => { await reviewOpportunity(opportunity.id, { decision: "reject", reason: rejectionReasons[opportunity.id].trim() }); return `已拒绝：${opportunity.title}`; })}>拒绝：{opportunity.title}</button></div></div> : opportunity.review_status === "rejected" ? <p><strong>拒绝原因：</strong> {opportunity.rejection_reason}</p> : <p>人工批准时间：{opportunity.reviewed_at ?? "时间不可用"}。</p>}
    </li>)}</ol>}
  </main>;
}
