...
- Final short-shop evidence job `fb293aa0…` truthfully persisted `available_count_observed=2`, kept the collection cap at three, then succeeded at exact available `2/2`, `natural_end_reached=true`, `sample_complete=true`, `shop_complete=false`. All 23 artifacts exist and all 22 SHA-bearing artifacts match current bytes. The other qualified account received clean replacement job `8c369a80…`, which succeeded at 3/3 with 34 artifacts and 33/33 SHA matches. Both results remain opportunity-eligible after restart; historical jobs remain unchanged.
...
- This checkpoint was open at the time and is superseded by the short-shop/CDP verification above. Its failed jobs remain unchanged; the current Phase A decision-loop status is recorded above. Phase B remains unstarted.
...
- At this historical checkpoint, the prescreen/artifact/restart portion was proven live while ranked Android continuation and the all-qualified-account run remained incomplete because the phone was not attached. The current status is recorded above; Phase B remains unstarted.
...
- At this historical checkpoint, Phase A overall UAT was still open because the then-current Android XML artifacts had a byte-hash limitation; the old malformed candidate was not approved, rejected, deleted or rewritten. The clean replacement evidence and current decision-loop result are recorded above. Phase B remains unstarted.
...
- At this historical checkpoint, the candidate had not been approved or rejected and overall UAT remained open for explicit human review plus the then-current XML byte-hash limitation. The later corrected analysis and clean evidence result are recorded above. No product/content/ZIP or other Phase B action has started.
