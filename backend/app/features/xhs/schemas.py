...
def persist_exact_account_result(
    ...
    *,
    ...
    if profile is None:
        ...
        session.add(profile)
    _apply_collection_item(profile, profile_item, binding)
    ...
    for item in note_items:
        note = XhsAccountNoteRecord(note_id=_note_id(item), user_id=user_id)
        _apply_collection_item(note, item, binding)
        session.add(note)
        notes.append(note)
    session.flush()
...
def _exact_account_items(result: CollectionResult) -> tuple[CollectionItem, list[CollectionItem]]:
    ...
    note_ids = [_note_id(item) for item in notes]
...
def _note_id(item: CollectionItem) -> str:
    ...
    if (
        ...
        raise AccountEvidencePersistenceError("Note identity is not exact.")
    return note_id


def _apply_collection_item(
